...
- [x] 1.1 Define the `TimerApi` interface (`setTimeout`, `clearTimeout`, `now`, `random`) and a
  ...
  1 000 draws.
- [x] 1.2 Build a test-only `TimerApi` harness backed by vitest fake timers with a settable
...
- [x] 1.3 Add an ESLint `no-restricted-globals` entry forbidding `setTimeout`, `setInterval`,
...
- [x] 2.1 Create `src/pod/snapshot.ts` with the layered `raw` / `overlay` / `effective` model and
...
- [x] 2.2 Model per-class state as unknown-until-first-success, and prove a later failure does not
...
- [x] 2.3 Derive and expose `connection` (`online`, `consecutiveFailures`, `lastSuccessAt`,
...
- [x] 2.4 Prove reads are synchronous and I/O-free: verify with a unit test that `get()` returns a
...
- [x] 3.1 Define the watched-field enumeration and the discriminated-union change type (per side:
...
- [x] 3.2 Implement batched per-commit notification computed on the effective view; verify with a
  ...
  post-commit value.
- [x] 3.3 Implement subscribe/unsubscribe; verify with a unit test that an unsubscribed listener
...
- [x] 3.4 Exclude the continuously-varying counter: verify with a unit test that two commits
...
- [x] 3.5 Implement listener isolation and re-entrancy queueing; verify with a unit test that when
...
- [x] 4.1 Implement overlay install/refresh/clear as commits, restricted to the overlayable fields
...
- [x] 4.2 Implement suppression: an observation disagreeing with an active entry must not change
...
- [x] 4.3 Implement retirement by agreement, including deriving power agreement from
...
- [x] 4.4 Implement expiry on an injected timer, dropping the entry and emitting the reversion when
...
- [x] 4.5 Verify an agreed entry never reverts: a fake-timer test where agreement arrives before
...
- [x] 5.1 Create `src/pod/poller.ts` with the endpoint-class descriptor registry and the four
...
- [x] 5.2 Implement the interval bounds — configured minimums of 5 000 ms (`pollIntervalMs`) and
...
- [x] 5.3 Implement ±10 % jitter drawn from the injected `random`; verify with a unit test that
...
- [x] 5.4 Implement self-rescheduling from poll settle, and prove in-flight suppression; verify
...
- [x] 6.1 Implement `requestMode(classId, { intervalMs, untilMs, reason })` returning a release
...
- [x] 6.2 Wire the priming mode: hold a fast mode while an observation reports `isPriming`, release
...
- [x] 6.3 Implement backoff as `clamp(effective × 2^failures, effective, max(maxBackoffMs,
  ...
  effective interval.
- [x] 6.4 Implement `bootstrap()` — all enabled classes in parallel, resolves on completion or at
...
- [x] 6.5 Implement `refresh(classId)`, attaching to an in-flight poll rather than issuing a second
...
- [x] 6.6 Implement `stop()`; verify with a test that after stopping no timer remains scheduled
...
- [x] 6.7 Prove nothing escapes the injected clock: verify with a test supplying a `TimerApi` whose
...
- [x] 7.1 Create `src/pod/writeQueue.ts` with the four lanes (`left`, `right`, `device`,
...
- [x] 7.2 Implement trailing-edge debounce with field-level merging; verify with the **#10
  ...
  and nothing else.
- [x] 7.3 Prove last-write-wins per field and lane independence; verify with unit tests that three
...
- [x] 7.4 Implement `writeMaxDebounceMs`; verify with a fake-timer test that a submission every
...
- [x] 8.1 Implement the reduction — keep `secondsRemaining` when non-zero, keep `isOn` otherwise —
...
- [x] 8.2 Prove equivalence against the mock as oracle: for each of the four combinations, verify
  ...
  get wrong.
- [x] 8.3 Prove the client's pre-flight rejection is never tripped: verify with a test that drives
...
- [x] 8.4 Prove single-field patches are untouched: verify with a unit test that a patch carrying
...
- [x] 9.1 Implement the global FIFO mutex across all lanes and both write endpoints; verify with a
...
- [x] 9.2 Implement `runExclusive(fn)`; verify with a test that a write becoming due during an
  exclusive section is recorded only after the section completes, and that a throwing section still
  releases the mutex.
- [x] 9.3 Install overlay entries at submission for the overlayable fields; verify with a unit test
...
- [x] 9.4 Re-base overlay expiry to dispatch-settle time and clear entries on dispatch failure;
...
- [x] 9.5 Request the fast poll on success only, via the injected `requestFastPoll` callback; verify
...
- [x] 9.6 Verify the queue holds no policy: a test seeding `settings.left.awayMode = true` asserts a
...
- [x] 9.7 Implement `stop()`; verify with a test that submissions still inside their debounce window
...
- [x] 10.1 Write `test/pollBudget.test.ts` reproducing design.md's scenario table — bootstrap at
...
- [x] 10.2 Add the sharp form of the invariant to the same test: verify the mock's recorded request
...
- [x] 11.1 Wire an end-to-end fake-timer test that runs poller, snapshot and write queue together
...
- [x] 11.2 Verify module boundaries hold: `grep` shows no import of `homebridge`, `hap-nodejs`, or
...
- [x] 11.3 Verify the four CI gates: `npm run lint`, `npm run typecheck`, `npm test` and
