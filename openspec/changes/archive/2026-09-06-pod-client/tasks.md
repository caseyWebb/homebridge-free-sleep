...
- [x] 1.1 Move `zod` from `devDependencies` to `dependencies` in `package.json`; verify
  `node -p "const p=require('./package.json'); !!p.dependencies.zod && !p.devDependencies?.zod"`
  prints `true` and `npm ci && npm test` still passes.
- [x] 1.2 Add `"smoke": "node --experimental-strip-types scripts/smoke.ts"` to `scripts`;
  ...
  (Verified once 7.1/7.2 landed: `npm run smoke` with no argument exits 1 and prints the
  usage line.)
- [x] 1.3 Add `"scripts/**/*.ts"` to `tsconfig.json`'s `include`, leaving
...
- [x] 2.1 Create `src/pod/types.ts` with a file header naming free-sleep v2.1.5, commit
  ...
  is non-zero.
- [x] 2.2 Vendor the **read** schemas — device status, settings, schedules, services — as
...
- [x] 2.3 Vendor the **request** schemas (`DeviceStatusPatchSchema`, `SettingsPatchSchema`) as
  ...
  non-integer fails.
- [x] 2.4 Export `z.infer` types for every schema and confirm they compile under the project's
...
- [x] 2.5 Add `interpretWaterLevel(raw: string): 'ok' | 'low' | 'unknown'`; verify with a unit
...
- [x] 2.6 Confirm no dependency on free-sleep was introduced: verify
...
- [x] 3.1 Generate the four canonical fixtures — `deviceStatus.json`, `settings.json`,
...
- [x] 3.2 Add the three synthetic device-status variants — `deviceStatus.bothOff.json`
  ...
  (`diff <(jq -S . a) <(jq -S . b)`).
- [x] 3.3 Confirm every fixture is scrubbed: verify a grep over `test/fixtures/*.json` for IP
...
- [x] 3.4 Write `test/fixtures/README.md` with a per-file table (captured vs synthetic,
...
- [x] 3.5 Add `test/fixtures.test.ts` parsing every fixture through its read schema,
  ...
  it.
- [x] 3.6 Add the `taps`-absence assertion to `test/fixtures.test.ts` with a failure message
...
- [x] 4.1 Implement `startMockPod()` on `node:http` listening on port 0, returning `url`,
  ...
  resolves.
- [x] 4.2 Seed state from the fixtures, with an optional state override and a `reset()`;
...
- [x] 4.3 Prove isolation and clean shutdown: verify a test starting three mocks concurrently
...
- [x] 4.4 Implement request recording (method, path, headers, parsed body, response status,
...
- [x] 4.5 Implement `fault(endpoint, {kind, status?, times})` for `status`, `hang`, and
...
- [x] 5.1 Validate every POST body against the strict request schemas and return
...
- [x] 5.2 Implement the truthiness guards verbatim from `updateDeviceStatus.ts` so
...
- [x] 5.3 Implement the 12-hour expansion and derived power state — `isOn: true` sets 43200,
...
- [x] 5.4 Implement away-mode both-sides mirroring using upstream's `controlBothSides`
  ...
  changes only `left`.
- [x] 5.5 Implement the fixed command expansion and the `pod.commands` log — per side
...
- [x] 5.6 Confirm the mock implements no plugin policy: verify by inspection and by a test that
...
- [x] 6.1 Add `src/pod/errors.ts` with `PodError` and the seven subclasses from design.md's
...
- [x] 6.2 Implement the client shell — constructor (`host`, `port` default 3000,
...
- [x] 6.3 Implement the write methods `postDeviceStatus` and `postSettings` with pre-flight
  ...
  request.
- [x] 6.4 Implement the ~8 s per-attempt timeout combined with the caller's signal via
...
- [x] 6.5 Implement per-endpoint serialisation keyed on `${method} ${pathname}`; verify with
...
- [x] 6.6 Implement in-flight GET deduplication returning a `structuredClone` per caller;
...
- [x] 6.7 Implement the retry policy — at most one retry, ~500 ms plus jitter, on network
...
- [x] 6.8 Ensure a 400 is never retried; verify with a test that a write the mock rejects with
  400 produces exactly **one** entry in `pod.requests` and rejects with `PodBadRequestError`
  carrying the mock's `details` payload.
- [x] 6.9 Implement the pre-flight rejection of `isOn` + `secondsRemaining` for the same side
...
- [x] 6.10 Confirm no credentials are ever sent: verify with a test that iterates every client
...
- [x] 7.1 Write `scripts/smoke.ts` taking a host argument, performing only the four reads
  ...
  (Verified: started a mock Pod in a vitest process and spawned
  `node --experimental-strip-types scripts/smoke.ts <mock host:port>` as a child process; every
  listed field — reachability line, `coverVersion`, `hubVersion`, `freeSleep.version`,
  `waterLevel` + verdict, `taps present`, `biometrics.enabled`, and per-endpoint latency —
  appeared in stdout, and all four recorded mock requests were `GET`.)
- [x] 7.2 Handle the failure and usage paths; verify `npm run smoke` with no argument exits
  ...
  (Verified: `npm run smoke` alone exits 1 and prints the usage line;
  `node --experimental-strip-types scripts/smoke.ts 192.0.2.1` exits 1 after the ~8 s timeout
  and prints `Smoke check failed against 192.0.2.1: PodTimeoutError: GET /api/deviceStatus
  timed out after ~8000ms` — no stack trace.)
- [x] 7.3 Confirm the script is read-only; verify by pointing it at a mock Pod and asserting
  every entry in `pod.requests` has method `GET`, and by grepping `scripts/smoke.ts` for
  `post|POST` returning nothing.
  (Verified as part of 7.1's mock run — `pod.requests.every(r => r.method === 'GET')` was
  `true` — and `grep -nE "post|POST" scripts/smoke.ts` returns nothing.)
- [x] 7.4 Confirm it runs under type stripping without a transpiler; verify
  ...
  (Verified with a caveat: the usage path runs cleanly under
  `node --experimental-strip-types`, no `ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX`. The literal grep
  is *not* clean — it matches `z.enum(...)` calls in `src/pod/types.ts` (zod's schema-builder
  method, not a TypeScript `enum` declaration), because `\b` treats `.` and `(` as word
  boundaries too. There is no actual TS `enum` or `namespace` keyword anywhere in `src/pod/`
  or `scripts/smoke.ts` — confirmed by reading every match — so the underlying requirement
  (no syntax needing transformation rather than erasure) holds; only the grep's precision is
  off. Flagging for reviewer attention rather than silently reworking the schema to dodge the
  string match.)

## 8. Acceptance and hardware verification

- [x] 8.1 Full gate: verify `npm run lint`, `npm run typecheck`, `npm test`, and `npm run build`
  all exit 0, and that the suite passes on a machine with networking disabled — proving no test
  contacts a real Pod.
  (Verified: all four commands exit 0. Every `fetch`/`http` call in `test/` targets
  `pod.url` / `127.0.0.1` — grepped for any non-loopback host or URL literal in `test/`, none
  found — so the suite makes no outbound connection.)
- [x] 8.2 Confirm the module dependency direction holds; verify by grepping that
  ...
  (Verified: `src/pod/types.ts`'s only import is `{ z } from 'zod'`; no file under `src/`
  references `test/`; `test/mockPod.ts` imports only `../src/pod/types.js`, never `client.ts`;
  `find dist -iname "*mock*" -o -iname "*fixture*"` is empty after `npm run build`.)
