...
- [x] 1.1 Copy the two real captures from
  ...
  scratchpad captures.
- [x] 1.2 Add both to `test/fixtures/README.md`'s provenance table as "captured, verbatim,
...
- [x] 2.1 In `src/pod/types.ts`, add `PresenceSideSchema` (`{ present: z.boolean(),
...
- [x] 2.2 In the same file, add `VitalsRecordSchema` (`{ id: z.number(), side: z.string(),
...
- [x] 2.3 New fixture-parsing test (mirrors the existing per-endpoint fixture tests) asserting
...
- [x] 3.1 Add `PodClient.getPresence(signal?: AbortSignal): Promise<PresenceData>` — `GET
...
- [x] 3.2 Add `PodClient.getVitals(query?: { side?: Side; startTime?: string; endTime?: string
...
- [x] 4.1 Extend `EndpointClassId` to `'deviceStatus' | 'settings' | 'schedules' | 'services' |
...
- [x] 4.2 Register the `'presence'` class: `baseIntervalMs: 30_000`, `read` calls
...
- [x] 4.3 Register the `'vitals'` class: `baseIntervalMs: 60_000`, `read` calls
...
- [x] 4.4 Verify the `enabled` predicate's existing re-evaluation-every-tick behavior (already
...
- [x] 5.1 Add `raw.presence: PresenceData | undefined` and `raw.vitals: VitalsResponse |
...
- [x] 5.2 Add private per-side proof-of-life state: `presenceBaseline: Record<Side, string |
  undefined>` and `presenceProven`/`vitalsProven: Record<Side, boolean>`, initialized to
  `undefined`/`false`. Verify: `npm run typecheck`.
- [x] 5.3 Implement `observePresence(data: PresenceData)`: inside `commit()`, for each side with
...
- [x] 5.4 Implement `observeVitals(records: VitalsResponse)`: inside `commit()`, for each side,
...
- [x] 5.5 Extend `EffectiveSideStatus` with `presencePresent`, `presenceActive`,
...
- [x] 5.6 Add `'presencePresent' | 'presenceActive' | 'vitalsOccupied' | 'vitalsActive'` to
...
- [x] 6.1 Add `GET /api/metrics/presence` to `test/mockPod.ts`'s known-endpoints switch,
  ...
  fixture.
- [x] 6.2 Add `GET /api/metrics/vitals` with real query filtering matching upstream's Prisma
  ...
  multi-row state.
- [x] 6.3 Add `StartMockPodOptions.state.presence` (whole-document override, mirroring
  ...
  reads reflect it.
- [x] 6.4 Extend `reset()` to restore `state.presence`/`state.vitals` to the initial snapshot,
...
- [x] 7.1 Create `src/services/occupancy.ts` exporting `OccupancySensorService`,
...
- [x] 7.2 Implement the source switch: read `ctx.config.occupancySource`; for `'presence'` use
...
- [x] 7.3 Implement `refresh()`: recompute both characteristics from `ctx.snapshot.get()` and
...
- [x] 8.1 Change `enabledServiceKeysFor`'s signature to `(hap, role, occupancySource)`; for a
...
- [x] 8.2 In `constructServicesFor`, construct an `OccupancySensorService` per side alongside
...
- [x] 8.3 In `handleSnapshotChanges`, add a branch: `isOccupancyChange(change) &&
...
- [x] 8.4 Pass `parsed.data.occupancySource` into the `PodPoller` constructor call alongside the
...
- [x] 9.1 In `src/config.ts`, move `occupancySource`'s doc comment from "Reserved — see module
...
- [x] 9.2 Update `config.schema.json`'s `occupancySource` field: drop "(reserved, no effect
...
- [x] 9.3 Update `README.md`'s config table row for `occupancySource`: drop "Reserved — no
...
- [x] 9.4 Extend `test/config.test.ts`'s existing "reserved key with a valid value is preserved
...
- [x] 10.1 Run all four CI gates locally per `CLAUDE.md` (`npm run lint`, `npm run typecheck`,
  `npm test`, `npm run build`) and confirm all pass. No live Pod contact at any point — mock Pod
  and fixtures only.
- [x] 10.2 Extend `test/integration/session.test.ts` (or wherever the whole-platform mock
