...
- [x] 1.1 Add `setAlarmVibrating(side: Side, value: boolean): void` to `test/mockPod.ts`'s
...
- [x] 1.2 Confirm (add an assertion if not already covered) that `test/mockPod.ts`'s existing
...
**Deviation (tech-lead resolution 3, design.md's final "Resolutions" section, dated after this
tasks.md was authored):** the module named `src/pod/scheduleTime.ts` below, and the derivation
functions in group 3, were combined into a single module, `src/pod/alarmSchedule.ts` — resolution
3 supersedes this file's original two-module split, requiring the *entire* pure next-alarm
derivation (eligibility, day-shift, override-skip, timezone) to live in one file so
`settings-switches`' later "Skip Next Alarm" computation has one shared module to import, per the
tech lead's explicit instruction. `AlarmWindowScheduler` (`src/pod/alarmWindowScheduler.ts`)
correspondingly imports only `alarmSchedule.ts`, `snapshot.ts`, and `poller.ts`, keeping only
window/timer/`requestMode` concerns, matching this task group's own original intent for that
module either way.

- [x] 2.1 Add a small utility module (e.g. `src/pod/scheduleTime.ts`) implementing the
...
- [x] 2.2 Implement the calendar-day shift (`jobs/utils.ts`'s `isEndTimeNextDay`: shift the
...
- [x] 2.3 Unit-test the timezone utility directly: for a timezone with a known, fixed UTC offset
...
- [x] 2.4 Add the DST-transition pinned test flagged in design.md's Open Question 3: for a
...
- [x] 3.1 Implement the eligibility predicate (`power.enabled && alarm.enabled &&
...
- [x] 3.2 Implement the regular-occurrence derivation: for each side, each weekday, combine
  ...
  day").
- [x] 3.3 Implement the `scheduleOverrides.alarm.expiresAt` suppression rule: a regular
...
- [x] 3.4 Implement the override one-shot instant: when `disabled` is false and both
...
- [x] 3.5 Compose 3.1–3.4 into one function, `deriveUpcomingAlarms(schedules, settings, now):
...
- [x] 4.1 Create `src/pod/alarmWindowScheduler.ts` exporting an `AlarmWindowScheduler` class,
...
- [x] 4.2 Implement the per-tick recompute-and-reconcile logic (design.md, "Recomputation"):
...
- [x] 4.3 Implement the self-rescheduling timer: `clamp(timeUntilSoonestUnarmedWindowStart,
...
- [x] 4.4 Implement `stop()`: cancel the scheduled timer and release every currently active
  ...
  after `stop()`.
- [x] 4.5 If `src/pod/{snapshot,poller,writeQueue}.ts`'s `no-restricted-globals`/
...
- [x] 5.1 In `src/config.ts`, update `alarmPollIntervalMs`'s doc comment from "Reserved for #16"
...
- [x] 5.2 Update `README.md`'s "Config keys" table: `alarmPollIntervalMs`'s row loses any
  "reserved/no effect yet" language. Verify: reading the rendered table.

## 6. `AlarmService` (HomeKit services)

- [x] 6.1 Create `src/services/alarm.ts` exporting `AlarmService`, constructor `(ctx:
...
- [x] 6.2 Construct the `StatelessProgrammableSwitch` service (name `` `${accessory.displayName}
...
- [x] 6.3 Construct the `Switch` "Dismiss Alarm" service (subtype `alarm-dismiss`) with `onGet`
...
- [x] 6.4 Implement `handleChange(change: Change)`: if `change.field === 'isAlarmVibrating' &&
...
- [x] 6.5 Implement the dismiss switch's `onSet`: `false` calls
...
- [x] 6.6 Implement the ~500 ms accept-then-revert timer for an on-write, reusing
...
- [x] 6.7 Implement the B1 initial-publish call from the constructor (mirrors
...
- [x] 6.8 Implement `stop()` clearing any pending revert timer, mirroring
  `ThermostatService.stop()`. Verify: unit test asserting no timer remains after `stop()` even
  with a revert pending.

## 7. Platform wiring

- [x] 7.1 In `src/platform.ts`, construct one `AlarmWindowScheduler` per launch alongside
...
- [x] 7.2 Add `alarmWindowScheduler.stop()` to the existing `api.on('shutdown', ...)` handler
...
- [x] 7.3 Add `ALARM_PRESS_SUBTYPE`/`ALARM_DISMISS_SUBTYPE` to `enabledServiceKeysFor`'s
  ...
  requirement).
- [x] 7.4 In `constructServicesFor`, construct one `AlarmService` per side role (never for the
...
- [x] 7.5 In `handleSnapshotChanges`, add a branch routing an `isAlarmVibrating` change to that
...
- [x] 7.6 Add `thermostat.stop()`-style cleanup for each `AlarmService` in the shutdown handler
...
- [x] 8.1 `test/config.test.ts`: assert `alarmPollIntervalMs`'s default (`3000`), minimum
...
- [x] 8.2 `test/platform.wiring.test.ts`: assert `alarmPollIntervalMs` is threaded from config
  into the constructed `AlarmWindowScheduler`. Verify: `npm test`.

## 9. End-to-end integration

- [x] 9.1 Extend `test/mockPod.test.ts` or `test/integration/session.test.ts`: seed a schedule
...
- [x] 9.2 Integration case for the pre-existing-vibration guard: bootstrap the platform against
...
- [x] 9.3 Integration case for schedule-edit responsiveness: start with no near-term alarm,
  ...
  `npm test`.
- [x] 9.4 Extend the platform's mock-Pod request-budget guardrail test (`platform` spec's "A
...
**Resolved:** tech lead resolution 1 (design.md's final "Resolutions" section) approved this
group as immediate work — `away-mode-guard` is merged into `main`, mooting the sequencing
question 10.1 originally asked. 10.2's own spec delta was written against `openspec/specs/
pod-write-queue/spec.md`'s *current* text (`openspec/changes/alarm-events/specs/pod-write-queue/
spec.md`) — that main spec is itself still the pre-`away-mode-guard` text (`away-mode-guard`'s own
delta has not yet been synced/archived into main specs, a pre-existing gap this change did not
create and does not attempt to close beyond its own one additional carve-out; see that delta
file's own "Context" section).

- [x] 10.1 Before starting this group, confirm with the tech lead (or check whether
...
- [x] 10.2 If resolved to fix here: in `WriteQueue.dispatch()`, treat a side patch whose only
...
- [x] 11.1 Run all four CI gates locally per `CLAUDE.md` (`npm run lint`, `npm run typecheck`,
