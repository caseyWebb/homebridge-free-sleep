...
- [x] 1.1 In `src/config.ts`, add `keepAliveMs` (positive integer, default `43_200_000`,
...
- [x] 1.2 Update `config.schema.json`: drop "(reserved, no effect yet)" from `keepAlive`'s
...
- [x] 1.3 Update `README.md`'s "Config keys" table: `keepAlive`'s row loses "Reserved — no
...
- [x] 2.1 Create `src/pod/keepAlive.ts` exporting a `KeepAlive` class per design.md's "A new,
...
- [x] 2.2 Implement the derived cadence: check interval
...
- [x] 2.3 Implement the per-tick decision for each side: read `snapshot.get()[side]` fresh: skip
...
- [x] 2.4 Implement `start()`/constructor behavior: schedule nothing at all when `enabled` is
...
- [x] 2.5 Route all timer access through the injected `TimerApi` (`src/pod/snapshot.ts`) —
...
- [x] 3.1 In `src/platform.ts`, construct a `KeepAlive` alongside the existing `poller`/
...
- [x] 3.2 Add `keepAlive?.stop()` to the existing `api.on('shutdown', ...)` handler, alongside
...
- [x] 4.1 `test/config.test.ts`: add cases for `keepAliveMs`/`keepAliveThresholdMs` defaults,
...
- [x] 4.2 New `test/keepAlive.test.ts`: using `createManualTimers()`/`createTimerHarness()`
...
- [x] 4.3 Same harness: a side observed on with `secondsRemaining` at or above the threshold
...
- [x] 4.4 Same harness: a side observed off (including one whose `secondsRemaining` was
  ...
  Verify: `npm test`.
- [x] 4.5 Same harness: after one re-arm fires, advance fake time by less than
...
- [x] 4.6 Same harness: construct `KeepAlive` with `enabled: false` and a side already on and
...
- [x] 4.7 Same harness: call `stop()` (a) before the first tick and (b) after several ticks;
...
- [x] 4.8 Mock-Pod-as-oracle integration case in `test/mockPod.test.ts` or
...
- [x] 4.9 Reduction-interaction case (documents the accepted race from design.md's Risks,
...
- [x] 5.1 Run all four CI gates locally per `CLAUDE.md` (`npm run lint`, `npm run typecheck`,
...
- [x] 6.1 Add the write origin (`'user'` default | `'keepAlive'`) to `submitSide` and drop
