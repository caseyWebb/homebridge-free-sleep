...
- [ ] 1.1 Define the zod schema with `host` required/non-empty and `sides` as
  ...
  `sides`).
- [ ] 1.2 Define the seven reserved keys (`pollIntervals`, `writeSettleMs`,
...
- [ ] 1.3 Add per-key range/enum constraints on the reserved keys (e.g. `writeSettleMs` a
...
- [ ] 1.4 Export `z.infer` types for the parsed config; verify `npm run typecheck` exits 0.
- [ ] 1.5 Add a normalization step for `host` (trim whitespace); verify with a unit test that
  `"  10.0.0.5  "` parses to `"10.0.0.5"`.

## 2. Homebridge UI schema (`config.schema.json`)

- [ ] 2.1 Create `config.schema.json` with `pluginAlias: "FreeSleep"` (matching
  ...
  key-by-key.
- [ ] 2.2 Add a unit test that loads both `config.schema.json` and the zod schema's shape
...
- [ ] 2.3 Add a unit test asserting `config.schema.json`'s `pluginAlias` equals the exported
...
- [ ] 3.1 Implement a fake `API` exposing `hap` (the real `@homebridge/hap-nodejs` `uuid`,
...
- [ ] 3.2 Implement `simulateRestart(platformFactory, previousAccessories)`: constructs a
...
- [ ] 3.3 Implement a fake `PodClient` exposing `getSettings()` as a resolved, rejected, or
...
- [ ] 4.1 Implement `seed(host, role)`, `uuid(host, role)`, and `serialNumber(host, role)`
...
- [ ] 4.2 Set `AccessoryInformation` (`Manufacturer`, `Model`, `SerialNumber`,
...
- [ ] 4.3 Set `Categories.THERMOSTAT` for side accessories and `Categories.OTHER` for the
...
- [ ] 5.1 Implement the constructor's config parse and bail-without-host guard: on parse
  ...
  accessories.
- [ ] 5.2 Implement `configureAccessory` collecting cached accessories by UUID; verify with
...
- [ ] 5.3 Implement the `sides: 'both'` fresh-install path: compute the three wanted
...
- [ ] 5.4 Implement restart-without-duplication: when all wanted accessories are already
  ...
  input.
- [ ] 5.5 Implement prune-on-restore: remove any service on a cached accessory whose
...
- [ ] 5.6 Implement the `sides` filter and unregistration of the excluded side; verify with
...
- [ ] 6.1 Implement the `needsSettings` guard (true only when at least one side accessory
...
- [ ] 6.2 Implement seeding a new side accessory's display name from
...
- [ ] 6.3 Implement the fallback name path when the settings read rejects, errors, or does
  ...
  time.
- [ ] 6.4 Implement never-rename-on-restore: an already-cached accessory's `displayName` is
...
- [ ] 7.1 Implement `src/index.ts` calling `api.registerPlatform(PLUGIN_NAME, PLATFORM_NAME,
...
- [ ] 7.2 Confirm `package.json`'s `main` (`dist/index.js`) resolves to this module after a
...
- [ ] 8.1 Full gate: verify `npm run lint`, `npm run typecheck`, `npm test`, and
  `npm run build` all exit 0.
- [ ] 8.2 Confirm no test contacts a real Pod or a real Homebridge process: verify the whole
