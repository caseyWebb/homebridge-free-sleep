...
- [ ] 1.1 Add `ServerStatusSchema` (a fifth top-level block, per design.md's Decision 8) plus a
      ...
      version/commit provenance matching the file's existing citation style. Include the six
      ...
      `systemDate`, `temperatureSchedule`) as required and the six biometrics-gated ones
      (`analyzeSleepLeft`, `analyzeSleepRight`, `biometricsInstallation`, `biometricsStream`,
      `biometricsCalibrationLeft`, `biometricsCalibrationRight`) as `.optional()`. Update the
      module doc's "Four blocks" table to "Five blocks" and note the deviation inline. Verify:
      a unit test in `test/types.test.ts` parses `test/fixtures/serverStatus.json` (task 7.1)
      successfully and rejects a document with a wrong-typed `status` value.
- [ ] 1.2 Add a strict `AlarmRequestSchema` (`side`, `vibrationIntensity` 1–100,
      `vibrationPattern` `'double' | 'rise'`, `duration` 0–180, `force`), citing
      `server/src/db/schedulesSchema.ts`'s `AlarmJobSchema` and `docs/POD-API.md`'s documented
      bounds. Export the inferred `AlarmRequest` type. Verify: a unit test asserts an
      out-of-bounds `vibrationIntensity` (e.g. `0` or `101`) fails validation.
- [ ] 1.3 Widen `DeviceStatusPatchSchema` is already `.partial()` over `isPriming: z.boolean()`
...
- [ ] 2.1 Widen `DevicePatch` to `Partial<{v, gainLeft, gainRight, ledBrightness, isPriming}>`.
...
- [ ] 2.2 Add a unit test asserting a device-lane submission merging both `isPriming` and a
...
- [ ] 2.3 Add `deviceWriteDebounceMs` to `WriteQueueOptions` (default 500, matching design.md's
...
- [ ] 2.4 Confirm (via existing test re-run, no code change expected) that `settleWrite`'s
...
- [ ] 3.1 Add `getServerStatus(signal?): Promise<ServerStatus>` following the existing
...
- [ ] 3.2 Add a `retry` option threaded through `request`/`requestWithRetry` (defaulting to
...
- [ ] 3.3 Verify `postAlarm` still serializes through the existing per-endpoint `enqueue`
...
- [ ] 4.1 Add a `serverStatus: ServerStatus | undefined` raw-state slot, an
...
- [ ] 4.2 Add a derived `serverFault: boolean` to `EffectiveSnapshot` — computed as "any
...
- [ ] 4.3 Expose `documents.serverStatus` alongside the existing `documents.deviceStatus/
...
- [ ] 5.1 Register a fifth endpoint class, `'serverStatus'`, on `slowPollIntervalMs`, wired to
...
- [ ] 5.2 Add a unit test asserting the `serverStatus` class's effective interval tracks
...
- [ ] 6.1 Add `test/fixtures/serverStatus.json`: all twelve always-present subsystems
      ...
      the choice in `test/fixtures/README.md`). Verify: `test/fixtures.test.ts` parses it
      through `ServerStatusSchema` (task 1.1) and asserts every subsystem's status is not
      `'failed'`.
- [ ] 6.2 Add `GET /api/serverStatus` to `KNOWN_ENDPOINTS` and the request switch: serve
      ...
      scenarios.
- [ ] 6.3 Add `POST /api/alarm` to `KNOWN_ENDPOINTS` and the request switch: validate against
...
- [ ] 6.4 Confirm `applyDeviceStatusPatch`'s existing `if (patch.isPriming)` guard already
...
- [ ] 7.1 Add four new boolean keys — `primeSwitch`, `ledLightbulb`, `testAlarmSwitch`,
...
- [ ] 7.2 Add `deviceWriteDebounceMs` to `PollIntervalsFieldsSchema` (`z.number().int().min(500,
...
- [ ] 7.3 Update `waterLowSensorType`'s doc comment from "Reserved — see module doc" to
...
- [ ] 7.4 Add the same six fields (four booleans, `deviceWriteDebounceMs`'s title/default under
...
- [ ] 8.1 `src/services/waterLow.ts`: `WaterLowService`, subtype `waterLow`, publishing either
...
- [ ] 8.2 `src/services/prime.ts`: `PrimeService`, subtype `prime`, a `Switch` with `On`'s
      ...
      scenarios.
- [ ] 8.3 `src/services/led.ts`: `LedService`, subtype `led`, a `Lightbulb` with `On` and
...
- [ ] 8.4 `src/services/testAlarm.ts`: `TestAlarmService`, subtype `testAlarm`, a momentary
      `Switch`. `onSet(true)` calls `ctx.podClient.postAlarm(...)` (threading `PodClient`
      through `ServiceContext` — see task 9.2) with `force: true` and a fixed short
      `vibrationIntensity`/`vibrationPattern`/`duration`, and — regardless of the promise's
      outcome — schedules a ~1 s `ctx.timers.setTimeout` pushing `On` back to `false`; `stop()`
      clears that timer. Verify: a fake-timer unit test in `test/services/testAlarm.test.ts`
      asserts the tile resets to `off` after ~1 s both when the write succeeds and when it
      rejects, matching `specs/hub-accessory/spec.md`'s "self-resets... regardless of outcome"
      scenario.
- [ ] 8.5 `src/services/serverFault.ts`: `ServerFaultService`, subtype `serverFault`, a
...
- [ ] 9.1 Grow `enabledServiceKeysFor('hub', hap)` to include the water-low sensor's
      ...
      `${UUID}:${subtype}` only when its config key is `true`. Verify: a unit test in
      ...
      scenario.
- [ ] 9.2 Add `podClient: MinimalPodClient` (widened, task 3.2, to include `postAlarm`) to
      ...
      it.
- [ ] 9.3 Grow `constructServicesFor('hub', ...)` to always construct `WaterLowService` and to
      conditionally construct `PrimeService`/`LedService`/`TestAlarmService`/
      `ServerFaultService` behind their respective config keys, storing each in a new private
      field (mirroring `this.connectionService`) for `handleSnapshotChanges` and `shutdown` to
      reach. Verify: an integration-style test starting the platform with all four keys enabled
      asserts all five hub services (plus connection) exist on the hub accessory; a second
      test with all four disabled asserts only connection and water-low exist.
- [ ] 9.4 Extend `handleSnapshotChanges` to route `'isPriming'` to `PrimeService.refresh()`,
      ...
      "no published service watches these fields yet" comment. Verify: a unit test asserts a
      snapshot change to each of the three fields calls only its own service's `refresh()`
      unaffected by the others, matching `specs/platform/spec.md`'s unmodified "A side change
      reaches only that side" pattern applied to these three new device-level routes.
- [ ] 9.5 On `api.on('shutdown', ...)`, call `stop()` on `PrimeService` and `TestAlarmService`
      (the two services owning a revert/reset timer) alongside the existing
      `poller`/`writeQueue`/`keepAlive`/`thermostat` stops. Verify: a shutdown test asserts no
      pending timer remains after firing shutdown with a refused prime-off write and an
      in-flight test-alarm trigger both outstanding, mirroring `away-mode-guard`'s own
      `platform.wiring.test.ts` precedent for `ThermostatService`'s revert timer.

## 10. Full verification

- [ ] 10.1 Run `npm run lint`, `npm run typecheck`, `npm test`, and `npm run build` locally and
      confirm all four succeed with every module, wiring, and test above in place (per
      `CLAUDE.md`, these are the CI gates).
- [ ] 10.2 Run `openspec validate hub-accessory --type change --strict` and confirm it passes
      with no errors.
