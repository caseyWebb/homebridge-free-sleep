...
- [x] 1.1 Create `src/services/types.ts` with the `ServiceContext` interface from design.md
...
- [x] 1.2 Add the enabled-subtype table to `src/platform.ts`: side accessories enable the
...
- [x] 2.1 Create `src/services/thermostat.ts` that adds (or restores by
...
- [x] 2.2 Declare all properties during construction: `TargetHeatingCoolingState` with
...
- [x] 2.3 Assert construction ordering: every `setProps` call happens before any value is set and
...
- [x] 3.1 Implement the five `onGet` handlers, all synchronous, reading only
...
- [x] 3.2 Implement the sticky ±1 °F deadband for `CurrentHeatingCoolingState` exactly as
...
- [x] 3.3 Implement the unknown-snapshot fallback: when the device-status class reads as unknown,
...
- [x] 3.4 Implement `TemperatureDisplayUnits`: lazy seed inside `onGet` from
...
- [x] 4.1 Implement `onSet(TargetHeatingCoolingState)` → `writeQueue.submit(side, {isOn})` and
...
- [x] 4.2 Make each write handler await its submission promise and throw
...
- [x] 4.3 Verify write coalescing end to end: a mode write and a setpoint write to the same side
...
- [x] 5.1 Implement `accessory.context.publishedF` (`{targetF?, currentF?}`) and the single-writer
  ...
  second.
- [x] 5.2 Make `onSet(TargetTemperature)` claim the degree into the shadow without calling
...
- [x] 5.3 Verify shadow persistence: a `simulateRestart` through the fake Homebridge API, followed
...
- [x] 5.4 Verify the failed-write revert: when a dispatch fails and the write queue clears its
...
- [x] 6.1 Create `src/services/connection.ts` adding (or restoring by `getServiceById`) a
...
- [x] 6.2 Map `connection.online` → `CONTACT_DETECTED` / `CONTACT_NOT_DETECTED` and `StatusFault` →
...
- [x] 6.3 Implement `StatusActive`: `false` until the first successful observation of the launch,
  ...
  stays `true`).
- [x] 6.4 Implement the `noResponseAfterMs` escalation as the lazy read-time predicate from
...
- [x] 6.5 Verify no timer is created for escalation: after `stop()`, the manual clock reports zero
  pending callbacks owned by the services, and the vitest process exits without an open-handle
  warning.

## 7. Platform wiring

- [x] 7.1 Construct the snapshot store, poller and write queue once in `src/platform.ts`, wire
...
- [x] 7.2 Order `didFinishLaunching` as: plan accessories → `await poller.bootstrap()` → construct
...
- [x] 7.3 Subscribe to `snapshot.subscribe` exactly once and dispatch each change per design.md's
...
- [x] 7.4 Stop the poller, the write queue and the snapshot subscription on Homebridge shutdown.
...
- [x] 8.1 Add `test/manualTimers.ts` — a `TimerApi` with a virtual `now()`, a sorted pending-callback
...
- [x] 8.2 Add `test/integration/session.test.ts`: boot `FreeSleepPlatform` through
...
- [x] 8.3 Assert the read-burst guardrail: read every characteristic of all three accessories at
  t=0 and again at t=60 s, and verify the mock's recorded request count is unchanged across each
  burst (delta exactly 0).
- [x] 8.4 Assert the slider-drag guardrail: six `TargetTemperature` writes 50 ms apart to one side,
...
- [x] 8.5 Assert the outage guardrail: inject a connection-destroy fault on the device-status
  ...
  last-known ones.
- [x] 8.6 Assert the offline-then-escalate path in the same harness: hold the fault past
...
- [x] 9.1 Update `docs/HOMEKIT.md` to record the decisions this change fixes that were previously
...
- [ ] 9.2 Post the "what needs a real paired device" list from design.md as comments on issues #9
  ...
  view 11`.
  > Tracking note: file-side content is finalized (design.md's "What needs a real paired
  > device" section, unchanged). Per the implementing agent's instructions, posting the actual
  > issue comments is left to the tech lead; the checklist text is included in the PR's final
  > report instead of being posted directly.
- [ ] 9.3 Report the `writeSettleMs` key collision upward: `platform-foundation`'s reserved-key
  ...
  > Tracking note: verified already resolved on `main` — `src/config.ts`'s `writeSettleMs`
  > default is `15000`; `openspec/changes/platform-foundation/design.md`'s reserved-keys table
  > (line ~185) and `specs/config/spec.md` both show the corrected default/owner with an
  > explicit "Corrected at reconcile of `thermostat-and-offline`" note; `poller-and-write-queue`'s
  > own design.md (line ~385) and proposal.md independently agree (`15000`, "overlay window").
  > Exactly one artifact (`poller-and-write-queue`) now states the default and owner — no file
  > edit needed here. Per the implementing agent's instructions, confirming this with the tech
  > lead / posting is left to them; the summary is included in the PR's final report.
- [ ] 9.4 Note on issue #14 what this change already ships (the `publishedF` shadow, compare in °F,
  ...
  > Tracking note: the note's content is finalized (design.md's "The °F shadow" section,
  > "What #14 adds on top, deliberately deferred" paragraph, unchanged). Per the implementing
  > agent's instructions, posting to the issue is left to the tech lead; the note text is
  > included in the PR's final report instead of being posted directly.

## 10. Gates

- [x] 10.1 Run `npm run lint`, `npm run typecheck`, `npm test`, and `npm run build`; all four pass.
- [x] 10.2 Verify the published artifact stays clean: `npm run build` emits no `src/services/**`
  reference to `@homebridge/hap-nodejs`, and `dist/` contains no test file. Check by grepping
  `dist/`.
- [x] 10.3 Verify the whole suite runs with no network and no Pod on the LAN, and that the vitest
